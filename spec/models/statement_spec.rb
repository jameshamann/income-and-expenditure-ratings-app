...
RSpec.describe Statement, type: :model do
   ...
   it 'validates the presence of income, expenditure and month' do
    expect(Statement.new(income: {"0":{name: "Salary", value:1000.00}}, expenditure: {"0":{name: "Rent", value: 100}}, month: '2024-04-01', user_id:user.id)).to be_valid
   end

   it 'calculates the total_income of the when there is a single income type' do 
      statement = Statement.create(income: {"0":{name: "Salary", value:1000.00}}, expenditure: {"0":{name: "Rent", value: 100.00}}, month: '2024-07-01',user_id:user.id)
      expect(statement.total_income).to eq 1000.00
     end

   it 'calculates the total_income of the income types' do 
    statement = Statement.create(income: {"0":{name: "Salary", value:1000.00},"1":{name: "Dividents", value:150.50}}, expenditure: {"0":{name: "Rent", value: 100.00},"1":{name: "Food", value: 250.43}}, month: '2024-07-01',user_id:user.id)
